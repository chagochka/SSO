...
def upload():
    ...
    if request.method == 'POST':
        ...
        if file.filename == 'report.txt':
            return 'You need a file name: "report.txt"', 400  # ОТПРОВЛЯЕМЫЙ ФАЙЛ ДОЛЖЕН НАЗЫВАТьСЯ "report.txt"
        if file and allowed_file(file.filename):
            ...
            ###
            idrep = current_user.name
            sqlite_connection = sqlite3.connect('db/db.sqlite')
